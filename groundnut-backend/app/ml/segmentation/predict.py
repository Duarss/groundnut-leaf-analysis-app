...
def predict_infected_areas(seg_batch_01, label: str, thr=None, overlay_alpha=None, return_mask: bool = False,):
    ...
    pred = model.predict(seg_batch_01, verbose=0)[0]

    ci = int(CLASS_TO_INDEX[label])
    prob = pred[..., ci]

    thr = float(Config.SEG_MASK_THRESHOLD if thr is None else thr)
    bin_mask01 = (prob >= thr).astype(np.float32)
    bin_mask_u8 = (bin_mask01 > 0.5).astype(np.uint8)
    ...
    if return_mask:
        out["infected_mask_bin"] = bin_mask_u8
